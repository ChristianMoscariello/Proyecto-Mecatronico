...
// =============================
// --- Envío JSON unificado
// =============================
void sendJsonToGS(const JsonDocument &doc) {
  String payload;
...
void sendTelemetry(double lat, double lon, double alt, double speed, double heading, unsigned long ts) {
  ...
  d["current_wp"] = 0.0; d["total_wp"] = 0.0;
  doc["ts"] = ts;
  sendJsonToGS(doc);
}
...
void sendTrimsToGS() {
  ...
  doc["t"] = "TRIM_VALUES";
  JsonObject d = doc.createNestedObject("d");
  d["accel"] = trim_throttle; d["roll_lr"] = trim_roll; d["roll_fb"] = trim_pitch; d["rudder"] = trim_yaw; d["switch"] = trim_switch;
  sendJsonToGS(doc);
}
...
// =============================
// --- Procesamiento mensajes entrantes
// =============================
void handleIncomingJson(const String &jsonIn) {
  StaticJsonDocument<256> docIn;
  DeserializationError err = deserializeJson(docIn, jsonIn);
  if (err) {
    Serial.println("JSON inválido: " + jsonIn);
    return;
  }
  ...
  if (topic == "FIRE" || topic == "PERSON") {
    ...
    // Esperamos: { "t":"TRIM_UPDATE","d":{"axis":"roll","value":X} }
    String axis = docIn["d"]["axis"] | "";
    float value = docIn["d"]["value"] | 0.0;
    if (axis.length() > 0) {
      if (axis=="roll_lr") trim_roll = value;
      else if (axis=="roll_fb") trim_pitch = value;
      else if (axis=="rudder") trim_yaw = value;
      else if (axis=="accel") trim_throttle = value;
      else if (axis=="switch") trim_switch = value;
      saveTrims();
      sendTrimsToGS();  // confirmación
      Serial.printf("Trim %s actualizado a %.2f\n", axis.c_str(), value);
    }

  } else if (topic == "GET_TRIMS") {
    sendTrimsToGS(); // petición de refrescar trims

  } else {
    Serial.println("Comando desconocido: " + topic);
  }
}
...
void loop() {
  ...
  // --- Mensajes entrantes desde Raspberry
  if (SerialRPI.available()) {
    String jsonIn = SerialRPI.readStringUntil('\n');
    jsonIn.trim();
    if (jsonIn.length() > 0) handleIncomingJson(jsonIn);
  }
}
