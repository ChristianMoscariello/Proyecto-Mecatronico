...
void sendMavGoto(double lat, double lon, float alt) {
    ...
    Serial.println("--------------------------------------------------");
    Serial.println("📡 ENVIANDO GOTO A PIXHAWK");
    ...
    Serial.println("📤 GOTO → PAQUETE ENVIADO A PIXHAWK\n");
}
    ...
    if (mavlink_parse_char(MAVLINK_COMM_0, c, &msg, &status)) {
      ...
      switch (msg.msgid) {
        ...
        case MAVLINK_MSG_ID_SYS_STATUS: {
          ...
          mav_batt_voltage = sys.voltage_battery / 1000.0f;  // V
          mav_batt_current = sys.current_battery / 100.0f;   // A
          mav_batt_remaining = sys.battery_remaining;        // %

          // debug
          Serial.print("Batt: "); Serial.println(mav_batt_voltage);
...
void processIncomingJSON(const String &jsonIn, bool fromGS) {
  ...
  Serial.println("--------------------------------------------------");
  Serial.println("📥 [processIncomingJSON]");
  ...
  if (fromGS) {
    ...
    if (strcmp(type, "ACK") == 0) {
      ...
      if (id != "") handleAck(id);
      Serial.println("📩 ACK desde GS procesado");
      return;
    }
    ...
    if (strcmp(type, "ARM") == 0) {
      Serial.println("📥 GS → ARM recibido");
      Serial.println("🔧 Acción: cambiar a GUIDED y armar motores");
      ...
      // Enviar GUIDED
      Serial.println("📡 MAV → GUIDED");
      pixhawkSetModeGuided();

      delay(200);

      // Enviar ARM
      Serial.println("📡 MAV → ARM");
      pixhawkArm(true);

      Serial.println("✔ Comandos ARM enviados al Pixhawk");
      return;
    }
    ...
    if (strcmp(type, "DISARM") == 0) {
      Serial.println("📥 GS → DISARM recibido");
      Serial.println("🛑 Acción: desarmar (se procesa en FSM)");
    ...
    if (strcmp(type, "RETURN") == 0) {
      Serial.println("📥 GS → RETURN recibido");
      Serial.println("↩️ Acción: volver a HOME");
    ...
    // --------------------------------------------------------------
    // MISSION_COMPACT → Cargar misión + Takeoff nativo
    // --------------------------------------------------------------
    if (strcmp(type, "MISSION_COMPACT") == 0) {

      Serial.println("📦 GS → MISSION_COMPACT recibido");
      Serial.println("📌 Cargando misión…");
      ...
      Serial.println("✔ Misión cargada:");
      Serial.printf("   Altitud: %.1f m\n", mission.altitude);
      Serial.printf("   Spacing: %.1f m\n", mission.spacing);
      Serial.printf("   Vértices: %d\n", mission.polygon.size());
      Serial.printf("   Acción evento: %s\n", mission.event_action.c_str());

      generateMissionPath(mission);
      sendAckToGS(msgId);

      Serial.println("🚁 TAKEOFF NATIVO → GUIDED enviado");
      ...
      Serial.println("✔ Iniciado TAKEOFF");
      return;
    }
  }
  ...
  // ------------------------------------------------------------------
  if (strcmp(type, "GO") == 0) {
    Serial.println("📥 RPi → GO recibido");
    analysisResult = GO;
    return;
  }

  if (strcmp(type, "FIRE") == 0) {
    Serial.println("📥 RPi → FIRE recibido");
    analysisResult = FIRE;
    ...
    Serial.printf("🔥 Confianza fuego: %.2f\n", confidence);
  ...
  if (strcmp(type, "PERSON") == 0) {
    Serial.println("📥 RPi → PERSON recibido");
    ...
    Serial.printf("🧍 Confianza persona: %.2f\n", confidence);

    sendEventToGS("PERSON", mav_lat, mav_lon, mav_alt_rel, millis(), confidence);
    return;
  }

  Serial.printf("⚠️ Tipo de mensaje desconocido: %s\n", type);
}
  ...
  while (SerialRPI.available() > 0) {
    ...
    if (!receivingJson) {
      if (c == '{') {
        ...
        Serial.println("📥 [RPi] Inicio JSON detectado");
      }
      continue;
    }
    ...
    if (receivingJson) {
      ...
      if (c == '}') {
        ...
        Serial.print("📥 [RPi] JSON completo recibido: ");
        Serial.println(rpiBuffer);
        ...
        if (!err) {
          ...
          Serial.println("📤 [RPi] JSON validado: " + jsonStr);
...
void handleLoRa() {
  ...
  if (size) {
    Serial.printf("📡 [LoRa] Paquete recibido (%d bytes)\n", size);
    while (LoRa.available()) {
      char c = (char)LoRa.read();
      loraRxBuf += c;
    }
    Serial.println("📥 [LoRa] Buffer: " + loraRxBuf);
  }

  String js;
  while (extractNextFrame(loraRxBuf, js, GS_HDR)) {
    Serial.println("📥 [LoRa] Frame extraído: " + js);
...
// ============================================================================
  ...
  switch (state) {
    ...
    case NAVIGATE: {
      ...
      Serial.println("--------------------------------------------------");
      Serial.printf("🧭 ESTADO: NAVIGATE | WP actual: %d de %d\n",
                    currentWaypoint, (int)pathPoints.size() - 1);

      if (currentWaypoint >= (int)pathPoints.size()) {
        Serial.println("🏁 Todos los waypoints completados → RETURN_HOME");
        state = RETURN_HOME;
        sendStatusToGS(state);
        break;
      }
      ...
      double dist  = haversineDistance(mav_lat, mav_lon, target.lat, target.lon);
      double speed = mav_ground_speed;

      Serial.printf("📍 Actual:  %.7f , %.7f\n", mav_lat, mav_lon);
      Serial.printf("🎯 Target:  %.7f , %.7f\n", target.lat, target.lon);
      Serial.printf("📏 Dist WP: %.2f m\n", dist);
      Serial.printf("💨 Speed:   %.2f m/s\n", speed);

      bool close_enough = dist < 3.0;
      bool slow_enough  = speed < 0.5;

      if (close_enough) Serial.println("👍 Cerca del WP (<3m)");
      if (slow_enough)  Serial.println("🐌 Velocidad baja (<0.5 m/s)");
      ...
      if (close_enough && slow_enough) {
        if (millis() - insideRadiusSince > 1500) {
          ...
          Serial.println("📸 Enviando STABLE a RPi…");
          sendStableToRPi(target);

          Serial.println("🔁 Pasando a STABILIZE…");
          state = STABILIZE;
          stateEntryTime = millis();
          analysisResult = NONE;
          sendStatusToGS(state);
      ...
      Serial.println("📡 Enviando GOTO → Pixhawk");
      sendMavGoto(target.lat, target.lon, mission.altitude);

      if (!mav_armed) {
        Serial.println("⚠️ FAILSAFE: desarmado en vuelo → ABORT");
        state = IDLE;
        sendStatusToGS(state);
      }
    ...
    // --------------------------------------------------------------------
    // 📷 STABILIZE (actualizado)
    // --------------------------------------------------------------------
    case STABILIZE: {
      unsigned long elapsed = millis() - stateEntryTime;

      if (elapsed < 350) {
        Serial.printf("📷 Estabilizando cámaras… (%lu ms)\n", elapsed);
        break;
      }

      Serial.println("📷 Cámaras estables → WAIT_ANALYSIS");
      state = WAIT_ANALYSIS;
    ...
    case WAIT_ANALYSIS:
      ...
      if (analysisResult != NONE) {
        ...
        Serial.println("📥 Resultado análisis RPi recibido");

        if (analysisResult == GO)     Serial.println("➡️ Orden RPi: GO");
        if (analysisResult == FIRE)   Serial.println("🔥 Orden RPi: FIRE");
        if (analysisResult == PERSON) Serial.println("🧍 Orden RPi: PERSON");

        if (analysisResult == PERSON) {
          Serial.println("🔧 Activando SERVO (PERSON detectado)");
          triggerServoAction();
        }

        if (analysisResult == FIRE || analysisResult == PERSON) {

          if (mission.event_action.equalsIgnoreCase("RETURN")) {
            Serial.println("↩️ event_action=RETURN → RETURN_HOME");
            state = RETURN_HOME;
          } else {
      ...
      if (millis() - analysisStartTime > ANALYSIS_TIMEOUT) {
        Serial.println("⌛ Timeout de análisis → continuar misión");
        currentWaypoint++;
        state = NAVIGATE;
        sendStatusToGS(state);
      }
    ...
    case RETURN_HOME: {
      ...
      Serial.printf("↩️ RETURN_HOME | Distancia a HOME: %.2f m\n", dist);

      sendMavGoto(mission.home.lat, mission.home.lon, mission.altitude);

      if (dist < 2.0 && mav_ground_speed < 0.5) {
    ...
    case COMPLETE:
      ...
      // ============================================
      // 🧠 RESULTADOS DESDE RPi (GO / FIRE / PERSON)
      // ============================================
      if (analysisResult != NONE) {
        ...
        if (analysisResult == GO)
          Serial.println("➡️ Orden RPi: GO");

        if (analysisResult == FIRE)
          Serial.println("🔥 Orden RPi: FIRE");

        if (analysisResult == PERSON)
          Serial.println("🧍 Orden RPi: PERSON");

        // ------------------------------
        // Procesamiento de cada resultado
        // ------------------------------
        if (analysisResult == GO) {
          ...
          Serial.println("🧍 PERSON detectado → SERVO");
          triggerServoAction();
        ...
        // Reset análisis para el siguiente WP
        analysisResult = NONE;
        break;
      }

      // ============================================
      // ⏳ Timeout sin detección de RPi
      // ============================================
      if (millis() - analysisStartTime > ANALYSIS_TIMEOUT) {
        Serial.println("⌛ Timeout de análisis → NAVIGATE");
        currentWaypoint++;
        state = NAVIGATE;
        sendStatusToGS(state);
      }
...
void simulateDroneMotion() {
  ...
  switch (state) {
    ...
    case IDLE:
      Serial.println("🧪 [SIM] Estado: IDLE");
      sendStatusToGS(state);
      break;

    case TAKEOFF:
      Serial.println("🧪 [SIM] Ascendiendo...");
      alt_f += 0.4;
    ...
    case NAVIGATE:
      if (currentWaypoint >= (int)pathPoints.size()) {
        Serial.println("🧪 🏁 [SIM] Fin ruta → RETURN_HOME");
        state = RETURN_HOME;
        sendStatusToGS(state);
        break;
      }
      ...
      // Movimiento simulado
      {
        ...
        Serial.printf("🧪 [SIM] Moviendo hacia WP%d (dist=%.1f)\n",
                      currentWaypoint, dist);

        if (dist < 1.5) {
          Serial.printf("🧪 ✔ [SIM] WP%d alcanzado\n", currentWaypoint);
          notifyWaypointReached(currentWaypoint);
          sendStableToRPi(target);
          state = STABILIZE;
          stateEntryTime = millis();
    ...
    case STABILIZE:
      if (millis() - stateEntryTime > 350) {
        Serial.println("🧪 📷 [SIM] Estabilizado → WAIT_ANALYSIS");
        state = WAIT_ANALYSIS;
        analysisStartTime = millis();
        sendStatusToGS(state);
      }
    ...
    case WAIT_ANALYSIS:
      Serial.println("🧪 [SIM] Esperando análisis RPi...");

      if (analysisResult != NONE) {
        Serial.println("🧪 📥 [SIM] Resultado análisis recibido");

        if (analysisResult == GO)
          Serial.println("🧪 ➡️ [SIM] Orden: GO");
        else if (analysisResult == FIRE)
          Serial.println("🧪 🔥 [SIM] Orden: FIRE");
        else if (analysisResult == PERSON)
          Serial.println("🧪 🧍 [SIM] Orden: PERSON");

        // Acción igual que en real
      ...
      if (millis() - analysisStartTime > ANALYSIS_TIMEOUT) {
        Serial.println("🧪 ⌛ [SIM] Timeout → NAVIGATE");
        currentWaypoint++;
        state = NAVIGATE;
        sendStatusToGS(state);
      }
    ...
    case RETURN_HOME:
      Serial.println("🧪 ↩️ [SIM] Volviendo HOME...");
    ...
    case LAND:
      Serial.println("🧪 🛬 [SIM] Aterrizando...");
      alt_f = max(alt_f - 0.15, 0.0);
      if (alt_f <= 0.5) {
        Serial.println("🧪 🟢 [SIM] LAND → COMPLETE");
        state = COMPLETE;
        sendStatusToGS(state);
      }
      break;
    ...
    case COMPLETE:
      Serial.println("🧪 🎉 [SIM] Misión completa");
      resetMissionState();
      sendStatusToGS(state);
      break;
  }
