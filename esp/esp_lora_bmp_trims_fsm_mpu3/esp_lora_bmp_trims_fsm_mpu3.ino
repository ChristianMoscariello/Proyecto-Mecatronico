...
void ensureValidMagCalibration() {
  bool bad = false;

  for (int i = 0; i < 3; i++) {
    if (isBadFloat(imuCal.magScale[i]) || imuCal.magScale[i] == 0.0f) bad = true;
  }

  if (bad) {
    Serial.println("⚠️ magScale inválido → usando identidad (1.0) y bias = 0");
    imuCal.magBias[0] = imuCal.magBias[1] = imuCal.magBias[2] = 0.0f;
...
void calibrateIMU_Static() {
  ...
  Serial.println("🧪 Calibración estática ACC+GYRO (quedarse quieto)");
  ...
  imuCal.accelBias[2] = (az/N) - 9.80665;
...
float tiltCompensatedYaw(float rollDeg, float pitchDeg, float mx, float my, float mz) {
...
void fuseYaw(float gz, float dt, float yawMag) {
  static float yawGyroInt = 0;
  yawGyroInt += gz * RAD_TO_DEG * dt;

  if (yawGyroInt < 0) yawGyroInt += 360;
  if (yawGyroInt >= 360) yawGyroInt -= 360;

  yawGyro = (1-ALPHA_YAW)*yawGyroInt + ALPHA_YAW*yawMag;
...
void updateIMU() {
  ...
  // Print
  unsigned long now = millis();
  if (now - lastAttPrint > ATT_PRINT_INTERVAL) {
    lastAttPrint = now;
    Serial.printf("\n--- IMU ---\n");
...
void computeMagCalibrationPRO() {
  ...
  Serial.println("🔎 Rangos crudos de MAG:");
  Serial.printf("   rangeX = %.3f uT (min=%.3f, max=%.3f)\n", rangeX, minX, maxX);
  Serial.printf("   rangeY = %.3f uT (min=%.3f, max=%.3f)\n", rangeY, minY, maxY);
  Serial.printf("   rangeZ = %.3f uT (min=%.3f, max=%.3f)\n", rangeZ, minZ, maxZ);

  const float MIN_RANGE = 5.0f;  // rango mínimo razonable

  bool badX = (rangeX < MIN_RANGE);
  bool badY = (rangeY < MIN_RANGE);
  bool badZ = (rangeZ < MIN_RANGE);

  if (badX || badY || badZ) {
    Serial.println("⚠️ Rango insuficiente en al menos un eje. Solo se actualizará bias, escalas = 1.0");

    imuCal.magBias[0] = (maxX + minX) * 0.5f;
    imuCal.magBias[1] = (maxY + minY) * 0.5f;
    imuCal.magBias[2] = (maxZ + minZ) * 0.5f;

    imuCal.magScale[0] = 1.0f;
    imuCal.magScale[1] = 1.0f;
    imuCal.magScale[2] = 1.0f;
  ...
  if (!isfinite(sx) || !isfinite(sy) || !isfinite(sz)) {
    Serial.println("❌ scale salió NaN/Inf, dejando escalas en 1.0 y solo aplicando bias");
    imuCal.magBias[0] = bx;
    imuCal.magBias[1] = by;
    imuCal.magBias[2] = bz;
    imuCal.magScale[0] = 1.0f;
...
void setup(){
  ...
  Serial.println("  M -> Test magnetómetro");
  Serial.println("  C -> Calibración 3D");
...
void loop(){
 ...
 // Serial.println("magqua");
  if (Serial.available()) {
      char c = Serial.read();
      if (c=='C') startMagCalibrationPRO(15000);
      if (c=='P') printMagCalibration();
      if (c=='M') {
        Serial.printf("MAG (uT): %.2f %.2f %.2f\n",
          mpu.mag_x_ut(),mpu.mag_y_ut(),mpu.mag_z_ut());
      }
    }
