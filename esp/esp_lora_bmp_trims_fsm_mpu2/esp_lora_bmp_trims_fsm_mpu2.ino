...
void processIncomingJSON(const String &jsonIn, bool fromGS) {
  ...
  if (fromGS) {
    ...
    // =============================================================
      ...
      startMagCalibrationAxis(30000);    // 30 s
...
void startMagCalibrationAxis(unsigned long durationMs) {
  if (!mpuReady) return;

  magCal.active = true;
  magCal.startMs = millis();
  magCal.durationMs = durationMs;
  magCal.samples = 0;

  magCal.x.minVal = magCal.y.minVal = magCal.z.minVal =  1e6;
  magCal.x.maxVal = magCal.y.maxVal = magCal.z.maxVal = -1e6;

  Serial.println("🧭 Iniciando calibración MAG por ejes... girar lentamente en los tres ejes ±180°");

  // Aviso inicial a la GS
...
void updateMagCalibrationAxis() {
  ...
  magCal.x.minVal = fminf(magCal.x.minVal, mx);
  magCal.x.maxVal = fmaxf(magCal.x.maxVal, mx);
  magCal.y.minVal = fminf(magCal.y.minVal, my);
  magCal.y.maxVal = fmaxf(magCal.y.maxVal, my);
  magCal.z.minVal = fminf(magCal.z.minVal, mz);
  magCal.z.maxVal = fmaxf(magCal.z.maxVal, mz);
  magCal.samples++;
  ...
  if (millis() - lastProgress > 2000) {
    ...
    JsonObject d = j.createNestedObject("d");
    d["phase"] = "RUN_AXIS";
    d["elapsed"] = (int)(millis() - magCal.startMs);
    d["samples"] = (int)magCal.samples;
    j["ts"] = (int)millis();
    sendJsonNoAckToGS(j);
  }

  // Verificar fin de calibración
  if (millis() - magCal.startMs > magCal.durationMs) {
    magCal.active = false;

    // Calcular bias y scale por eje
    float bx = (magCal.x.maxVal + magCal.x.minVal) / 2.0f;
    float by = (magCal.y.maxVal + magCal.y.minVal) / 2.0f;
    float bz = (magCal.z.maxVal + magCal.z.minVal) / 2.0f;

    float avgRange = ((magCal.x.maxVal - magCal.x.minVal) +
                      (magCal.y.maxVal - magCal.y.minVal) +
                      (magCal.z.maxVal - magCal.z.minVal)) / 3.0f;

    float sx = avgRange / (magCal.x.maxVal - magCal.x.minVal);
    float sy = avgRange / (magCal.y.maxVal - magCal.y.minVal);
    float sz = avgRange / (magCal.z.maxVal - magCal.z.minVal);
    ...
    // Calidad estimada (simetría simple)
    float symmetry = fminf(fminf(sx, sy), sz) / fmaxf(fmaxf(sx, sy), sz);

    const char* result = (symmetry > 0.75f ? "OK" :
                          symmetry > 0.6f ? "FAIR" : "POOR");

    Serial.printf("✅ MAG (axis) calibrado — Bias(%.2f,%.2f,%.2f) Scale(%.2f,%.2f,%.2f) → %s\n",
                  bx, by, bz, sx, sy, sz, result);
