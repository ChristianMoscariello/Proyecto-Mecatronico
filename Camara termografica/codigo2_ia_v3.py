...
- Analiza con YOLOv8 (best.onnx)
- Envía alertas específicas al ESP32:
      PERSON_CSI
      FIRE_CSI  (para fuego y humo)
      GO_CSI
"""
...
class Config:
    ...
    MODELO_ONNX: str = os.path.join(SCRIPT_DIR, 'best.onnx')
    ...
    def __post_init__(self):
        if self.CLASES is None:
            self.CLASES = ["Persona", "Fuego", "Humo"]
...
class AnalizadorIA:
    ...
    # === NUEVA FUNCIÓN — Retorna clases detectadas ===
    ...
    def _procesar_imagen(self, ruta_imagen):
        try:
            ...
            if not hubo_det:
                ...
                self.esp32.enviar_alerta("GO_CSI")
                return
            ...
            # Persona
            if 0 in clases:
                self.esp32.enviar_alerta("PERSON_CSI")

            # Fuego o Humo → mismo mensaje
            if 1 in clases or 2 in clases:
                self.esp32.enviar_alerta("FIRE_CSI")
