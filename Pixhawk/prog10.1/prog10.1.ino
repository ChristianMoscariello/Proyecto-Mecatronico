...
void processIncomingJSON(const String &jsonIn, bool fromGS) {
  ...
  if (fromGS) {
    ...
    if (strcmp(type, "MISSION_COMPACT") == 0) {
      ...
      // ==========================================================
      // GENERAR WAYPOINTS DENTRO DEL POLÍGONO (escaneo en serpentina)
      // ==========================================================

      double camWidth   = 5.0;   // metros de cobertura horizontal
      double camHeight  = 7.0;   // metros hacia adelante
      double overlap    = 0.20;   // 20% solapamiento
      int maxWps        = 20;    // límite de seguridad

      vector<Coordinate> polyWPs =
          generatePolygonWPs(mission.polygon, camWidth, camHeight, overlap, maxWps);

      pathPoints.clear();
      for (auto &p : polyWPs)
          pathPoints.push_back(p);

      // Seguridad: verificar generación
      if (pathPoints.empty()) {
          Serial.println("❌ ERROR: No se generaron waypoints dentro del polígono");
          mission.loaded = false;
          sendAckToGS(msgId);
          return;
      }

      Serial.printf("🧭 %d waypoints generados dentro del polígono\n",
                    (int)pathPoints.size());
...
// ============================================================================
// 🟦 GENERA WAYPOINTS DENTRO DE UN POLÍGONO (scan en serpentina)
// ============================================================================
vector<Coordinate> generatePolygonWPs(
    const vector<Coordinate>& polygon,
    double camWidth,
    double camHeight,
    double overlap,
    int maxWps
    ...
    double latMin = polygon[0].lat, latMax = polygon[0].lat;
    double lonMin = polygon[0].lon, lonMax = polygon[0].lon;

    for (auto& p : polygon) {
        latMin = min(latMin, p.lat);
        latMax = max(latMax, p.lat);
        lonMin = min(lonMin, p.lon);
        lonMax = max(lonMax, p.lon);
    }

    // Separación según footprint y overlap
    double spacing = camHeight * (1.0 - overlap);

    // Convertimos spacing a grados aprox: 1e-5° ~ 1.11 m
    double spacing_deg = spacing * 1e-5;

    int lines = (int)((latMax - latMin) / spacing_deg) + 1;

    bool reverse = false;

    for (int i = 0; i < lines && result.size() < maxWps; i++) {
        double lat = latMin + i * spacing_deg;

        Coordinate A { lat, lonMin };
        Coordinate B { lat, lonMax };

        if (reverse) std::swap(A, B);
        reverse = !reverse;

        result.push_back(A);
        if (result.size() >= maxWps) break;
        result.push_back(B);
    }
